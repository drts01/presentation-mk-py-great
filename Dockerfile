...
RUN cd && set -ex &&\
  dnf --assumeyes install aria2 findutils gcc git-core make zeromq-devel \
...
RUN cd && PATH="${HOME}/.local/bin:${PATH}" && set -ex &&\
  ...
  eval "$(pyenv init -)" &&\
  pyenv install --verbose 3.9-dev &&\
  pyenv install --verbose 3.6.10 &&\
  pyenv install --verbose 2.7.17 &&\
  pyenv install --verbose pypy3.6-7.3.0 &&\
  pyenv global 3.9-dev pypy3.6-7.3.0 3.6.10 &&\
  rm -rf -- ~/.cache
